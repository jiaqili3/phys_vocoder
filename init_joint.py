...
for i in range(len(config.model)):
    if config.model[i] == RawNet3:
        ...
        config.model[i].threshold = 0.3295809328556061
    elif config.model[i] == ECAPATDNN:
        config.model[i] = ECAPATDNN(**model_config['ECAPATDNN'])
        config.model[i].load_state_dict(torch.load('./pretrained_models/ECAPATDNN.pth'))
        config.model[i].threshold = 0.33709782361984253
    elif config.model[i] == ResNetSE34V2:
        config.model[i] = ResNetSE34V2(**model_config['ResNetSE34V2'])
        config.model[i].load_state_dict(torch.load('./pretrained_models/ResNetSE34V2.pth'))
        config.model[i].threshold = 0.3702884316444397
    elif config.model[i] == XVEC:
        config.model[i] = XVEC(**model_config['XVEC'])
        config.model[i].load_state_dict(torch.load('./pretrained_models/XVEC.pth'))
        config.model[i].threshold = 0.879676103591919
    elif config.model[i] == XVEC1:
        config.model[i] = XVEC1()
        config.model[i].load_state_dict(torch.load('./pretrained_models/XVEC1.pth'))
        config.model[i].threshold = 0.28246

# ---------------------------------------- Attack ---------------------------------------

config.attack = edict()
config.attack.adv_dir = '/mntcephfs/lab_data/lijiaqi/joint_adver_out/'
